...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if((uint32_t)src1 <  src2) s->dnpc = s->pc + imm;);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if((uint32_t)src1 >= src2) s->dnpc = s->pc + imm;);
  ...
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
	...
	INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu	 , I, R(rd) = ((uint32_t)src1 < src2)); 

	INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra 	 , R, R(rd) = (src1 >> src2)); 
	INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl 	 , I, R(rd) = ((uint32_t)src1 >> src2)); 
	INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll 	 , I, R(rd) = ((uint32_t)src1 << src2)); 
	...
	INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu	 , R, R(rd) = ((uint32_t)src1 % src2)); 
